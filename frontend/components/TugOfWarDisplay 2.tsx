...
// Word display mapping for better voice recognition
const teamWordDisplay: { [key: string]: string } = {
  'pull': 'pull hard',
  'heave': 'heave ho'
};

export default function TugOfWarDisplay({ players, teams, teamWords, ropePosition }: TugOfWarDisplayProps) {
  ...
  return (
    <div className="w-full bg-gradient-to-br from-purple-900/50 to-pink-900/50 backdrop-blur-xl rounded-3xl p-8 border border-white/10 shadow-2xl">
      <h3 className="text-3xl font-black mb-6 text-transparent bg-gradient-to-r from-red-400 to-blue-400 bg-clip-text text-center">
      ...
      <div className="grid grid-cols-2 gap-8 mb-8">
        ...
        <div className={`relative group transform transition-all duration-300 ${redCloseToWin ? 'scale-105' : ''}`}>
          ...
          <div className="relative bg-gray-900/70 backdrop-blur rounded-2xl p-6 border-2 border-red-500/50">
            ...
            <div className="bg-gradient-to-r from-red-400 to-pink-400 rounded-xl p-3 mb-4 text-center">
              <p className="text-lg font-semibold text-gray-900">Say:</p>
              <p className="text-3xl font-black text-gray-900">
                {teamWordDisplay[teamWords.red] || teamWords.red}
              </p>
            </div>
            
            <div className="space-y-2">
              {redTeamPlayers.map(player => (
                <div key={player.id} className="flex items-center gap-2 bg-red-900/30 rounded-lg p-2">
        ...
        <div className={`relative group transform transition-all duration-300 ${blueCloseToWin ? 'scale-105' : ''}`}>
          ...
          <div className="relative bg-gray-900/70 backdrop-blur rounded-2xl p-6 border-2 border-blue-500/50">
            ...
            <div className="bg-gradient-to-r from-blue-400 to-cyan-400 rounded-xl p-3 mb-4 text-center">
              <p className="text-lg font-semibold text-gray-900">Say:</p>
              <p className="text-3xl font-black text-gray-900">
                {teamWordDisplay[teamWords.blue] || teamWords.blue}
              </p>
            </div>
            
            <div className="space-y-2">
              {blueTeamPlayers.map(player => (
                <div key={player.id} className="flex items-center gap-2 bg-blue-900/30 rounded-lg p-2">
                  <span className="text-xl">💪</span>
                  <span className="text-white font-semibold">{player.name}</span>
                </div>
              ))}
